    ...
    <div className={`h-screen w-full flex items-center justify-center font-sans ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      <div className="w-full max-w-md p-8 animate-in fade-in zoom-in duration-500">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-black uppercase tracking-tighter mb-1 leading-tight text-blue-600">PT KARYAINDAH ALAM SEJAHTERA</h1>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] opacity-80">Security System Portal</p>
        </div>
  ...
  return (
    <aside className={`fixed lg:relative inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out border-r ${styles.sidebar} ${isOpen ? 'w-72 translate-x-0' : 'w-0 -translate-x-full lg:translate-x-0 lg:w-0 lg:border-none'}`}>
      <div className={`flex flex-col h-full w-72 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="p-4 flex-1 overflow-y-auto text-left">
        ...
        <div className={`p-4 border-t ${styles.border}`}>
          <div className="flex items-center gap-3 p-3 rounded-lg bg-blue-500/5 mb-2">
            <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-xs uppercase">{username?.charAt(0)}</div>
            <div className="flex flex-col text-left">
              <span className="text-[10px] font-bold opacity-50 uppercase tracking-tighter">OPERATOR</span>
              <span className="text-sm font-bold truncate">{username}</span>
            </div>
          </div>
          <button onClick={onSettingsClick} className={`w-full flex items-center gap-3 p-3 rounded-lg ${styles.hover} text-slate-400 hover:text-slate-100 transition-colors`}>
            <Settings size={18} />
            <span className="text-sm font-medium text-left">Pengaturan</span>
          </button>
          <button onClick={onLogout} className={`w-full flex items-center gap-3 p-3 rounded-lg ${styles.hover} text-rose-500 transition-colors active:scale-95`}>
            <LogOut size={18} />
...
// ==========================================
// 3. KOMPONEN MODAL SETTINGS (GANTI PASSWORD)
// ==========================================
const SettingsModal = ({ theme, isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      ...
      setError('Gagal memperbarui password. Periksa password lama Anda.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
      <div className={`${styles.card} rounded-[2rem] w-full max-w-md shadow-2xl p-8 border animate-in zoom-in`}>
        <div className="flex justify-between items-center mb-8">
          <h3 className="text-xl font-bold flex items-center gap-3 text-blue-500">
            <KeyRound size={22} /> Pengaturan Akun
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-left">
          <div className="space-y-2">
            <label className="text-[10px] font-bold opacity-60 uppercase tracking-widest">Password Lama</label>
            <input type="password" required className={`w-full ${styles.input} border rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-blue-500/50`} value={data.oldPassword} onChange={e => setData({...data, oldPassword: e.target.value})} />
          </div>
          <div className="h-px bg-slate-800 my-2"></div>
          <div className="space-y-2">
            <label className="text-[10px] font-bold opacity-60 uppercase tracking-widest">Password Baru</label>
            <input type="password" required className={`w-full ${styles.input} border rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-blue-500/50`} value={data.newPassword} onChange={e => setData({...data, newPassword: e.target.value})} />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-bold opacity-60 uppercase tracking-widest">Konfirmasi Password Baru</label>
            <input type="password" required className={`w-full ${styles.input} border rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-blue-500/50`} value={data.confirmPassword} onChange={e => setData({...data, confirmPassword: e.target.value})} />
          </div>

          {error && <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg text-xs text-rose-500">{error}</div>}
          
          <button disabled={isSaving} type="submit" className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 mt-4 uppercase text-xs tracking-widest">
          ...
          {activeServer ? (
            ...
            <div className="w-full h-full flex items-center justify-center p-8 animate-in fade-in duration-700">
              <div className={`${styles.card} p-14 rounded-[3rem] w-full max-w-lg border flex flex-col items-center text-center shadow-2xl relative overflow-hidden group`}>
                ...
                <div className="bg-blue-600/10 w-24 h-24 rounded-[2.5rem] flex items-center justify-center mb-10 group-hover:scale-110 transition-transform duration-500"><Server size={44} className="text-blue-500" /></div>
                ...
                <div className="grid grid-cols-2 gap-4 w-full mb-8">
                  <div className="p-4 bg-slate-800/30 rounded-2xl border border-slate-700/50"><p className="text-[10px] text-slate-500 uppercase font-bold mb-1 text-left">Total Unit</p><p className="text-2xl font-bold text-white text-left">{servers.length}</p></div>
                  <div className="p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/10"><p className="text-[10px] text-emerald-500/70 uppercase font-bold mb-1 text-left">Online</p><p className="text-2xl font-bold text-emerald-500 text-left">{servers.filter(s => s.status === 'online').length}</p></div>
                </div>
